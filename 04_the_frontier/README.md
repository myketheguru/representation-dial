...
| `access_search_split_v4.py` (4.0.0) | The **primary design**: structure-knob sweep at dim=2 with the rank metric (imports v1 + v3). | **Confirmed.** Hyperbolic MAP gap **+0.128 (pure tree) → −0.075 (random)** — strongly positive for hierarchy, declining monotonically as structure is destroyed. The pre-registered TRUE outcome: matched geometry helps, bounded by the structure present. |

**Run** (from inside this folder, needs numpy): `python access_search_split.py` · `python access_search_split_v2.py` · `python access_search_split_v3.py` · `python access_search_split_v4.py`.
